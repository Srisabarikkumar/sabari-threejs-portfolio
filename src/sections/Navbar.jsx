import {useState} from "react";
import {navLinks} from "../constants";
import GlitchText from "../components/HeroTextDesign.jsx";
import GradientText from "../components/HeroTextDesign.jsx";
import TrueFocus from "../components/TrueFocus.jsx";
import HomeIcon from "../components/AnimatedIcons/HomeIcon.jsx";

// const TrueFocusUtil = ({text}) => {
//     return (
//     <TrueFocus
//         sentence={text}
//         manualMode={false}
//         blurAmount={2}
//         borderColor="#00D8FF"
//         animationDuration={2}
//         pauseBetweenAnimations={1}
//     />
//     );
// }

const NavItems = () => {
    return (
        // <ul className="nav-ul">
        //     {navLinks.map(({id, href, name}) => (
        //         <li key={id} className="nav-li">
        //             <a href={href} className="nav-li_a"
        //                onClick={() => {}}>
        //                 {name}
        //             </a>
        //         </li>
        //     ))}
        // </ul>
        <HomeIcon />
    );
}


const Navbar = () => {
    const [isOpen, setIsOpen] = useState(false);

    const toggleMenu = () => {
        setIsOpen((prevIsOpen) => !prevIsOpen);
    }

    return (
        <header className="fixed top-0 left-0 right-0 z-50 backdrop-filter backdrop-blur-md">
            <div className="max-w-7xl mx-auto">
                <div className="flex justify-between items-center py-5 mx-auto c-space">
                    <a href="/" className="text-green-300 font-bold text-xl hover:text-white
                    transition-colors">
                      Srisabarikkumar M
                    </a>

                    <button onClick={toggleMenu} className="text-neutral-400 hover:text-white focus:outline-none
                    sm:hidden flex">
                        <img src={isOpen ? "/assets/close.svg" : "/assets/menu.svg"} alt="toggle" className="size-6"/>
                    </button>

                    <nav className="sm:flex hidden">
                        <NavItems />
                    </nav>
                </div>
            </div>

            <div className={`nav-sidebar ${isOpen ? 'max-h-screen' : 'max-h-0'}`}>
                <nav className="p-5">
                    <NavItems />
                </nav>
            </div>
        </header>
    )
}
export default Navbar
